...
- [`apps/db-visualizer/src/app/api/`](../../apps/db-visualizer/src/app/api/)
  - `GET /api/landing` — pages, sections, content graph, alien-filtered media
...
- Normalizers coerce unknown API shapes into safe view models: [`apps/db-visualizer/src/server/normalize-api-data.ts`](../../apps/db-visualizer/src/server/normalize-api-data.ts)

### Server-side fetch to own origin

- [`apps/db-visualizer/src/server/api-client.ts`](../../apps/db-visualizer/src/server/api-client.ts) resolves base URL from `x-forwarded-host` / `host` and `x-forwarded-proto` (Vercel-friendly) so route pages can call `/api/*` with `cache: "no-store"`.
...
- Landing loads **all** pages with nested sections and content items—fine for modest CMS size; revisit **pagination** or **lazy routes** if `shared` grows large.
...
| Pages / content | Unbounded `findMany` | [`landing.ts`](../../apps/db-visualizer/src/server/queries/landing.ts) |
...
2. Add `GET /api/...` route with `force-dynamic` + `no-store`.
3. Extend [`normalize-api-data.ts`](../../apps/db-visualizer/src/server/normalize-api-data.ts) if the payload is new.
4. Add or extend a panel and CSS module under `src/components/`.
5. Update **this doc** and, if needed, [`packages/database.md`](../packages/database.md) or platform ERDs.
